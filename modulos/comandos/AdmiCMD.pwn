...
cmd:crearempresa(playerid, params[])
{
	if(personajeInfo[playerid][p_spawn] == true)
	{
		...
		{
			new nombre[80], tipo, vw;
			if(sscanf(params, "s[80]ii", nombre, tipo, vw)) return SendClientMessage(playerid, -1, ""Chat_Violeta"[error]"Chat_Gris" /crearempresa [nombre] [tipo] [vw]");
			{
				new Float:x, Float:y, Float:z;
				GetPlayerPos(playerid, x, y, z);
				new xylos[300];
				mysql_format(xylosMYSQL, xylos, sizeof(xylos), "INSERT INTO Empresas(e_nombre,e_posx, e_posy, e_posz, e_posx1, e_posy2, e_posz3,e_tipo, e_vw) VALUES ('%s', '%f', '%f', '%f', 1577.2582, 6.6992, 917.9416,'%d', '%d')", nombre, x, y, z, tipo, vw);
				mysql_pquery(xylosMYSQL, xylos);
				SendClientMessage(playerid, -1, ""Chat_Blanco"[MiamiSide]"Chat_Gris" Creaste una nueva empresa");
				for(new h;h<MAX_EMPRESAS;h++)
				{
					cargarEmpresas(h);
				}
			}
...
cmd:dempresa(playerid, params[])
{
	new id, idd;
	if(personajeInfo[playerid][p_spawn] == true)
	{
		if(masterInfo[playerid][_admin] <= 5) return SendClientMessage(playerid, -1, ""Chat_Rojo"[error]"Chat_Gris" No tienes permisos suficientes.");
		if(sscanf(params,"ii", id, idd)) return SendClientMessage(playerid, -1, ""Chat_Violeta"[error]"Chat_Gris"/dempresa [id] [id-empresa]");
		if(!IsPlayerConnected(id)) return SendClientMessage(playerid, -1, ""Chat_Violeta"[error]"Chat_Gris" jugador no conectado");
		{
			...
			format(string, 128, ""Chat_Blanco"[MiamiSide]"Chat_Gris"El administrador "Chat_Radio"%s"Chat_Gris" te dio lider de la empresa "Chat_Radio"%s"Chat_Gris"", conseguirNombre(playerid), empresaInfo[idd][e_nombre]);
			format(string1, 128, ""Chat_Blanco"[MiamiSide]"Chat_Gris"Le diste al jugador "Chat_Radio"%s"Chat_Gris" lider de la empresa "Chat_Radio"%s", conseguirNombre(id), empresaInfo[idd][e_nombre]);
			format(log1, sizeof(log1), "El administrador %s le dio la empresa %s a %s", conseguirNombre(playerid), empresaInfo[idd][e_nombre], conseguirNombre(id));
			SendClientMessage(playerid, -1, string1);
			SendClientMessage(id, -1, string);
			personajeInfo[id][p_empresa] = empresaInfo[idd][e_id];
			personajeInfo[id][p_empresarango] = 6;
			enviarNotificacionG(playerid, "Felicidades ahora eres lider de una empresa");
		}
	}
	return 1;
}
...
cmd:ddinero(playerid, params[])
{
	...
	if(personajeInfo[playerid][p_spawn] == true)
	{
		...
		if(!IsPlayerConnected(id)) return SendClientMessage(playerid, -1, ""Chat_Violeta"[error]"Chat_Gris" jugador no conectado");
		{
			...
			SendClientMessage(playerid, -1, string1);
			SendClientMessage(id, -1, string);
			enviarMoneyNotif(id, 0, cantidad);
...
cmd:ditem(playerid, params[])
{
	...
	if(personajeInfo[playerid][p_spawn] == true)
	{
		...
		if(!IsPlayerConnected(id)) return SendClientMessage(playerid, -1, ""Chat_Violeta"[error]"Chat_Gris" jugador no conectado");
		{
			...
			agregarIttem(playerid, item, cantidad);
		}
	}
	return 1;
}
